...
interface PartPaidFeesResponse {
  status: boolean;
  type: string;
  total: number;
  students: Student[];
...
export default function AdminPartPaidFeesPage() {
  ...
  // Fetch part paid fees when both session and term are selected
  useEffect(() => {
    if (sessionId && termId) {
      fetchPartPaidFees();
    } else {
  ...
  const fetchPartPaidFees = async () => {
    ...
    try {
      ...
      console.error("Error fetching part paid fees:", err);
      setError(
        err.response?.data?.message || "Failed to load part paid fees. Please try again."
      );
      setStudents([]);
      setTotal(0);
    } finally {
  ...
  return (
    <DashboardLayout role="admin">
      <Card>
        ...
        >
          <h1 style={{ margin: 0 }}>Students Who Paid Part Fees</h1>
