...
export default function AdminUnpaidFeesPage() {
  ...
  // Fetch unpaid fees when both session and term are selected
  useEffect(() => {
    if (sessionId && termId) {
      fetchUnpaidFees();
    } else {
  ...
  const fetchUnpaidFees = async () => {
    ...
    try {
      const response = await axios.get<UnpaidFeesResponse>(
        `http://127.0.0.1:8000/api/admin/fees/unpaid?session_id=${sessionId}&term_id=${termId}`,
        getAuthHeaders()
      );
      ...
      setError(
        err.response?.data?.message || "Failed to load unpaid fees. Please try again."
      );
      setStudents([]);
      setTotal(0);
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "parent_id",
      key: "parent_id",
      render: (parentId: number | null) => (parentId ? parentId : "N/A"),
    },
  ];
  ...
  return (
    <DashboardLayout role="admin">
      <Card>
        ...
        >
          <h1 style={{ margin: 0 }}>Students Who Haven't Paid Fees</h1>
