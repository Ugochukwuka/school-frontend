...
export default function ParentPayFeePage() {
  ...
  useEffect(() => {
    if (sessionId && termId && childUuid) {
      fetchFees();
    } else {
      setFees([]);
      setSelectedFees({});
    }
  }, [sessionId, termId, childUuid]);

  // Redirect to parent dashboard after 5 seconds on successful payment
  useEffect(() => {
    if (paymentSuccess) {
  ...
  const fetchChildren = async () => {
    if (!parentUuid) {
      setError("Unable to get parent information. Please log in again.");
      return;
    }
  ...
  const handleChildChange = (value: string) => {
    const child = children.find((c) => c.uuid === value);
    setChildUuid(value);
  ...
  const handlePayment = async () => {
    ...
    if (!selectedChild || !childUuid || !parentUuid) {
  ...
  return (
    <DashboardLayout role="parent">
      <Card>
        <h1 style={{ marginBottom: "24px" }}>Pay Fee</h1>
        ...
        {paymentSuccess && (
          <Alert
            title="Payment processed successfully! Redirecting in 5 seconds..."
            type="success"
            showIcon
            style={{ marginBottom: 20 }}
          />
        ...
        <Form form={form} layout="vertical">
          <div style={{ marginBottom: "24px" }}>
            <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", marginBottom: "20px" }}>
              ...
              <Form.Item label="Child" style={{ marginBottom: 0, minWidth: 250 }}>
                <Select
                  value={childUuid}
                ...
                >
                  {children.map((child) => (
                    <Select.Option key={child.uuid} value={child.uuid}>
                ...
                <div style={{ marginTop: "24px", padding: "16px", background: "#f5f5f5", borderRadius: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Button
                      ...
                      disabled={totalAmount <= 0 || processingPayment}
                    >
                      Process Payment
                    </Button>
                  </div>
